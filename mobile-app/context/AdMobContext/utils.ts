...
const checkIsNotAgreement = async () => {
  const {
...
export const loadGdprAdsConsent = async () => {
  try {
    ...
    if (data.isConsentFormAvailable) {
      ...
      if (data.status === "OBTAINED") {
        ...
        if (isNotAgreement) await AdsConsent.showForm();
      }

      if (resultForm.canRequestAds) initializeMobileAdsSdk();
    } else {
...
export const checkIsConsentAvailable = async () => {
  try {
    const data = await requestConsent();
...
export const showPrivacyOptionsForm = async () => {
  try {
    const data = await requestConsent();
    if (data.isConsentFormAvailable) await AdsConsent.showPrivacyOptionsForm();
  } catch (error) {
